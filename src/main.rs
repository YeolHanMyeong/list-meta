...
fn build_line(entry: &DirEntry, root_path: &str, meta: &Option<toml::Value>) -> Option<Line> {
    ...
    let permission = build_permission_prefix(&meta_data).unwrap();
    let nlink = meta_data.nlink().to_string();
    let user_name: String = get_user_by_uid(meta_data.uid()).map(|u| u.name().to_string_lossy().to_string()).unwrap_or_default();
    let group_name: String = get_group_by_gid(meta_data.gid()).map(|group| group.name().to_string_lossy().to_string()).unwrap_or_default();
...
fn load_folder_description(folder_path:&str, name:&str) -> String {
...
fn file_description(name:&OsString, value:&Option<toml::Value>) -> String {
...
fn build_permission_prefix(meta_data: &Metadata) -> std::io::Result<String> {
        let entry_type = entry_type_indicator(&meta_data);
        let permissions = meta_data.permissions();
        let permission_mode = format_permissions(&permissions.mode());

        Ok(format!("{entry_type}{permission_mode}"))
}

fn entry_type_indicator(meta_data: &Metadata) -> String{
    if meta_data.is_dir() {
        "d".to_string()
